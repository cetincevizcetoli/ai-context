...
KNOWN_BINARY_EXTENSIONS = {
    ...
    ".mp3", ".mp4", ".avi", ".mkv", ".mov", ".pdf",
    ".exe", ".dll", ".so", ".pyd", ".pyc",
    ".class", ".dat", ".db", ".sqlite", ".sqlite3"
}
...
def generate_tree_text(node, prefix=""):
    ...
    for d in dirs:
        ...
        child_text = generate_tree_text(child, prefix + "│   ")
        if child_text:
...
def write_report(root_path, tree_files, content_files, binary_files, skipped_dirs, args):
    ts = datetime.now().strftime("%Y-%m-%d %H:%M")
    report = io.StringIO()

    report.write(f"# 📜 PROJE DOKÜMÜ ({ts})\n")
    report.write(f"**Dizin:** `{root_path}` | **Dosya Sayısı:** {len(tree_files)}\n\n")

    report.write("## 📂 YAPISAL ÖZET\n```text\n")
    report.write(generate_tree_text(build_tree(tree_files, skipped_dirs)))
    report.write("\n```\n\n---\n")
    ...
    if not args.tree_only:
        ...
        for rel_path in tree_files:
            ...
            if rel_path in binary_set:
                report.write(f"\n### 📄 `{rel_path}`\n")
                report.write("```text\n[BINARY DOSYA - içerik okunmadı]\n```\n")
                continue

            if rel_path not in content_set:
                continue
    ...
    if args.tokens:
        print(f"Tahmini Baglam: ~{len(report_text) // 4} Token")

    if args.clipboard and copy_to_clipboard(report_text):
        print("Sonuc panoya kopyalandi.")
...
def main():
    ...
    for r, dirs, files in os.walk(root):
        ...
        for d in list(dirs):
            ...
            if args.git_ignore and not is_ex:
                for pattern in git_rules:
                    p = pattern.rstrip("/")
                    if (
                        fnmatch.fnmatch(rel_dir, p)
                        or fnmatch.fnmatch(d, p)
                        or rel_dir.startswith(p + "/")
                    ):
                        is_ex = True
                        break
        ...
        for f in files:
            ...
            if any(rel_p.startswith(sd + "/") for sd in skipped_dirs):
                continue
            ...
            if is_gi and rel_p not in force_include:
                continue
            ...
            if ext in KNOWN_BINARY_EXTENSIONS:
                tree_files.append(rel_p)
                binary_files.append(rel_p)
                continue
